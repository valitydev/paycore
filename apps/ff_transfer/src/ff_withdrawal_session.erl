...
-type action() ::
    undefined
    | continue
    | {setup_callback, ff_withdrawal_callback:tag(), prg_machine_action:timer()}
...
-spec map_action(action(), session_state()) -> prg_machine_action:t() | undefined.
...
map_action(finish, _Session) ->
    prg_machine_action:unset_timer();
