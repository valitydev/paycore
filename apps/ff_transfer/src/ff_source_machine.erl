...
create(Params, Ctx) ->
    ff_machine_lib:create(?NS, fun ff_source:create/1, Params, Ctx).
...
-spec get(id(), event_range()) ->
    ...
    | {error, notfound}.
get(ID, {After, Limit}) ->
    ff_machine_lib:get(?NS, ID, {After, Limit}, ff_source, notfound).
...
process_repair(Scenario, Machine) ->
    ff_machine_lib:process_repair(ff_source, Machine, Scenario).

-spec marshal_event_body(prg_machine:event_body()) -> {pos_integer(), binary()}.
marshal_event_body(Body) ->
    ff_machine_lib:marshal_event_body(source, ?EVENT_FORMAT_VERSION, Body).

-spec unmarshal_event_body(pos_integer(), binary()) -> prg_machine:event_body().
unmarshal_event_body(Format, Payload) ->
    ff_machine_lib:unmarshal_event_body(source, ?EVENT_FORMAT_VERSION, Format, Payload).
