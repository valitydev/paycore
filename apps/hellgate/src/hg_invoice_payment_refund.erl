...
do_process(finished, _Refund) ->
    {done, {[], progressor_action:new()}}.

process_refund_cashflow(Refund) ->
    Action = progressor_action:set_timeout(0, progressor_action:new()),
        ...
        {finished, ?session_failed(Failure)} ->
            case check_retry_possibility(Failure, Refund) of
                ...
                fatal ->
                    ...
                    {next, {Events1 ++ RollbackStarted, progressor_action:set_timeout(0, Action)}}
            end;
        _ ->
            {next, {Events1, Action}}
    end.
...
process_accounter(Refund) ->
    ...
    {done, {[?refund_status_changed(?refund_succeeded())], progressor_action:new()}}.
...
process_failure(Refund) ->
    ...
    {done, {Events, progressor_action:new()}}.
...
retry_session(Action, Timeout) ->
    ...
    NewAction = progressor_action:set_timer({timeout, Timeout}, Action),
    {NewEvents, NewAction}.
